...
export default function BottomBar() {
  ...
  return (
    <div className="fixed right-3 sm:right-6 bottom-6 sm:bottom-10 z-50 flex flex-col items-end gap-3 sm:gap-4">
      <a
        href={`https://wa.me/${whatsappNumber}`}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-green-500 shadow-lg hover:bg-green-400 transition"
      ...
      <a
        ...
        className="flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-yellow-500 shadow-lg hover:bg-yellow-400 transition"
        aria-label={`Call now ${phoneNumberDisplay}`}
      >
        <Phone size={24} className="sm:hidden text-white" />
        <Phone size={28} className="hidden sm:block text-white" />
      </a>
    </div>
  );
}
